...
foreach (array (200, 203, 403, 404, 500, 503) as $httpStatus) {
    $answer = ($answer && stdinSend (randomChannel (), '/sync/', $jsonData, 'number.zero', '==', '0', array ('response' => ($httpStatus . ' XHR test')), STDOUT_EXPECT_MATCH, array (STDERR_EXPECT_XHR_EVENT_LOADEND, sprintf (STDERR_EXPECT_XHR_STATUS_format, $httpStatus))));
}

msg_log ("  +---+---+ Synchronous mode for a failed request...");
$answer = ($answer && stdinSend (randomChannel (), $GLOBALS['invalidAddress'] . '/sync/', $jsonData, 'number.zero', '==', '0', array (), STDOUT_EXPECT_NOHELPER, STDERR_EXPECT_XHR_EXCEPTION));
...
foreach (array (200, 203, 403, 404, 500, 503) as $httpStatus) {
    $answer = ($answer && stdinSend (randomChannel (), '/', $jsonData, 'number.zero', '==', '0', array ('response' => ($httpStatus . ' XHR test')), STDOUT_EXPECT_MATCH, array (STDERR_EXPECT_XHR_EVENT_LOADEND, sprintf (STDERR_EXPECT_XHR_STATUS_format, $httpStatus))));
}
